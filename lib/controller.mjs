...
export function judge(state, { tool, command, path: target, root = null }, substitutionDepth = 0) {
  ...
  if (state.reproduction && line.trim() === String(state.reproduction.command).trim()) {
    return { allow: true, reason: "this is the session's recorded reproduction" };
  }
...
const COMMANDS = {
  ...
  scratch(opts, root) {
    ...
    save(state, root);
    process.stdout.write(`${pattern} is scratch; its churn no longer invalidates a verification\n`);
  },
  ...
  transition(opts, root) {
    ...
    if (target === "VERIFY") {
      ...
      state.verification = { at: new Date().toISOString(), diffHash: diffHash(root, state.scratch ?? []), gates: {} };
    }
