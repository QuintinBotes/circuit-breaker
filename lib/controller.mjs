...
export function judge(state, { tool, command, path: target, root = null }, substitutionDepth = 0) {
  ...
  for (const { name, args, raw } of leadingCommands(line)) {
    ...
    if (HUMAN_ONLY[sub] && (sub !== "init" || open)) {
...
const COMMANDS = {
  ...
  interrupt(opts, root) {
    ...
    let marked = null;
    if (entry && (entry.status === "open" || entry.status === "confirmed")) {
    ...
    process.stdout.write(
      marked
        ...
        : `interrupted in ${from}; no hypothesis was in play to mark, and the session is in ` +
          `HYPOTHESIZE\n`,
    );
  },
  ...
  status(opts, root) {
    ...
    const external = state.hypotheses
      .filter((h) => h.blames === "external" && h.status !== "rejected")
      .map((h) => {
  ...
  transition(opts, root) {
    ...
    if (target === "EXPERIMENT") {
      ...
      const ungrounded = state.hypotheses.filter(
        (h) => h.status === "open" && h.blames === "external" && !h.cites && !h.undocumented,
      );
      if (ungrounded.length) {
        process.stderr.write(
          `cb: ${ungrounded.map((h) => h.id).join(", ")} ` +
            `${ungrounded.length === 1 ? "blames" : "blame"} something outside this code and ` +
            `${ungrounded.length === 1 ? "cites" : "cite"} no mechanism. Before measuring, ` +
            `either name where the behaviour is written down with ` +
            `${CB} hypothesis ground <id> --cites <url|path[:line]>, or say on the record ` +
            `that there is nothing to name with --undocumented "<what you searched>". ` +
            `Rejecting the claim is the other way forward.\n`,
        );
        process.exit(1);
      }
    }
