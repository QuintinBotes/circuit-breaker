...
export function projectDir(hint) {
  ...
  const from = hint ? path.resolve(hint) : process.cwd();
...
export function diffHash(root = projectDir()) {
  ...
  try {
    ...
    try { fs.unlinkSync(scratch); } catch { /* nothing to remove */ }
  }
  ...
  if (failed) return UNKNOWN_TREE;
  hash.update(
    untracked.split("\n").filter((line) => line && !line.startsWith(".claude/circuit-breaker/")).join("\n"),
  );
...
export function withLock(root, mutate) {
  ...
  for (;;) {
    try { fs.mkdirSync(lock); break; } catch {
      ...
      try {
        if (Date.now() - fs.statSync(lock).mtimeMs > 5000) { fs.rmdirSync(lock); continue; }
 ...
 */
export function lexCommandLine(command) {
      ...
      if (ch === ">") {
        ...
        if (text[j] === "&") {
          ...
          if (!/^(\d+|-)(\s|$|[;|&])/.test(tail)) {
            ...
            if (!isDiscardTarget(target)) redirects = true;
          }
...
function isDiscardTarget(target) {
  ...
  return /^\/dev\/(null|stdout|stderr|fd\/\d+|tty)$/.test(clean);
}
...
export function classifyCommand(name, args, raw = name) {
  ...
  if (name === "git") {
    ...
    if (sub === "stash" || sub === "worktree") {
      return after[1] === "list" || after.length === 1 ? "read" : "write";
    }
 ...
 */
export function judge(state, { tool, command, path: target }, substitutionDepth = 0) {
  ...
  for (const inner of lexed.subs) {
    const verdict = judge(state, { tool: "Bash", command: inner }, substitutionDepth + 1);
