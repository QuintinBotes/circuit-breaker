 ...
 */
export const CB = `node ${JSON.stringify(fileURLToPath(new URL("../bin/cb", import.meta.url)))}`;
...
export function redirectsOutput(command) {
  ...
  for (let i = 0; i < text.length; i++) {
    ...
    if (ch === "\\") { i += 1; continue; }
    if (ch !== ">") continue;
    if (text[i + 1] === "&") continue; // `2>&1` duplicates a descriptor; it is not a file.
    // Discarding output is not writing. `2>/dev/null` is in half the diagnostic command
    // lines anybody writes, and a gate that refuses them makes OBSERVE unusable for the
    // sake of nothing, and OBSERVE is where a session spends most of its time.
    const target = text.slice(i + 1).replace(/^>?\s*/, "");
    if (/^\/dev\/(null|stdout|stderr)\b/.test(target)) {
      i += text.slice(i + 1).length - target.length;
      continue;
    }
    return true;
  }
...
function awkWrites(args) {
  // Every argument, not the first one: the command line was split on whitespace, so a
  // program is spread across as many words as it has spaces and `print > "f"` arrives as
  // three of them.
 ...
 */
export function cbSubcommand(name, args) {
  const nonFlag = args.filter((a) => !a.startsWith("-"));
  if (name === "cb" || /(^|[/\\])cb$/.test(name)) return nonFlag[0] ?? "";
  const runner = ["node", "bun", "deno"].includes(name);
  if (runner && nonFlag[0] && /(^|[/\\])cb$/.test(nonFlag[0].replace(/^["']|["']$/g, ""))) {
    return nonFlag[1] ?? "";
  }
  return null;
}
...
export function classifyCommand(name, args) {
  ...
  if (cbSubcommand(name, args) !== null) return "read";
...
export function judge(state, { tool, command, path: target }) {
  ...
  // Two commands let the agent out of the controller rather than through it, and neither is
  // reachable from inside a session. `cb end` releases the project outright. `cb init` is
  // worse disguised: it rewrites the state file empty, which drops the ledger, the active
  // fix and the verification, and re-stamps the diff hash so the Stop gate sees a tree that
  // was never changed. Both are denied while a session is open; starting one is not, since
  // an agent that chooses the discipline is doing the thing this plugin is for. A person
  // typing either in their own terminal meets no hook and is unaffected.
  const open = state.state !== "INACTIVE";
  for (const { name, args } of leadingCommands(line)) {
    const sub = cbSubcommand(name, args);
