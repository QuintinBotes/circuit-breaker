...
export function redirectsOutput(command) {
  ...
  for (let i = 0; i < text.length; i++) {
    ...
    // lines anybody writes, and a gate that refuses them makes OBSERVE — the state a
    // session spends most of its time in — unusable for the sake of nothing.
...
export function judge(state, { tool, command, path: target }) {
  ...
  for (const { name, args } of leadingCommands(line)) {
    if (name !== "cb" && !/[/\\]cb$/.test(name)) continue;
    const sub = args.find((a) => !a.startsWith("-")) ?? "";
        ...
        reason:
          sub === "end"
            ? `cb end closes the session and hands the project back unguarded, which is the ` +
              ...
              `it. Everything else — hypothesis, experiment, transition, gate — is yours.`
